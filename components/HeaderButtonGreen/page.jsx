...
export const HeaderButtonGreen = () => {
	...
	return (
		<div className="header">
			<div className="w-full bg-(--color-light-pink) text-(--color-light-black) text-[15px] font-semibold flex items-center justify-center md:justify-between gap-4 p-[20px] py-[10px] md:px-[40px]">
				<div className="flex gap-8">
					...
					<div className="hidden md:block">
						...
						<span>Mon – Sat: 8:30 am – 8:00 pm, Sun: Closed</span>
					</div>
				</div>
				<div className="flex gap-8">
					<div>
						...
						<span>
							<Link href="tel:1-800-123-1234">1-800-123-1234</Link>
						</span>
					</div>
			...
			<div className="w-full relative flex items-center justify-between gap-[20px] p-[20px] md:py-[30px] md:px-[40px] text-(--color-black)">
				<div className="w-[43%] flex max-[1100px]:hidden items-center gap-[40px]">
					<ul className="desktop-menu items-center gap-[20px] font-medium">
						<li className="menu-1 relative flex items-center gap-2 cursor-pointer">
							...
							<div className="subMenu-1 w-[90vw] absolute top-full left-[-40px] pt-5 z-10">
						...
						<li className="menu-2 relative flex items-center gap-2 cursor-pointer">
							...
							<ChevronDown className="w-[15px] h-[15px]" />
							<div className="subMenu-2 w-[600px] absolute top-full left-[-92px] pt-5 z-10">
										...
										<div className="flex flex-col gap-6 text-[15px]">
											{SERVICE_SELECT.map((item) => (
												<Link
													href={item.link}
													key={item.id}
													className="hover:text-(--color-pink)"
												>
													{item.service}
												</Link>
											))}
				...
				<div className="w-[43%] flex items-center justify-end gap-[40px] ml-auto">
					<div className="flex items-center gap-[20px]">
						<div className="cursor-pointer">
							<Search />
						</div>
						<div className="relative cursor-pointer">
							<div className="absolute top-[-5px] right-[-10px] flex items-center justify-center text-[12px] text-white w-[18px] h-[18px] bg-(--color-pink) p-1 rounded-full">
								0
							</div>
							<ShoppingCart />
						</div>
					...
					<button
						className="hidden md:block w-fit uppercase border border-(--color-green) bg-transparent text-(--color-green) hover:border-(--color-green) hover:bg-(--color-green) hover:text-white duration-300 px-[30px]"
						onClick={() => openDialog()}
					>
						Book a visit
					</button>
